...
var ContentFlowItem  = function (CFobj, element, index) {
    ...
    this.clickInactiveItem = function (event) {
        if(!event) var event = window.event;
    ...
    this.clickActiveItem = function (event) {
        if(!event) var event = window.event;
        var el = event.target ? event.target : event.srcElement;
        var index = el.itemIndex ? el.itemIndex : el.parentNode.itemIndex;
...
ContentFlowItem.prototype = {
    ...
    makeActive: function () {
        ...
        var caItem = this.clickActiveItem;
        if (window.addEventListener) {
            this[this._activeElement].addEvent('click', caItem, false);
        } else {
            this[this._activeElement].onclick = caItem;
        }
            
        CFobj._onMakeActive(this);
    },
    ...
    makeInactive: function () {
        ...
        var caItem = this.clickActiveItem;
        var ciItem = this.clickInactiveItem;
        if (window.addEventListener) {
            this[this._activeElement].removeEvent('click', caItem, false);
        } else {
            this[this._activeElement].onclick = ciItem;
        }

        CFobj._onMakeInactive(this);
    },

    initClickInactive: function () {
        var CFobj = this.CFobj;
        var ciItem = this.clickInactiveItem;
        if (window.addEventListener) {
            this[this._activeElement].addEvent('click', ciItem, false);
        } else {
            this[this._activeElement].onclick = ciItem;
        }
    },
        ...
        if (typeof index == "string") {
            switch (index) {
                ...
                default:
                    ...
                    if (index < this._activeItem.index) index++;
            }
        }
...
*/
    ...
    _init: function () {
        ...
        var flow = $(this.container).getChildrenByClassName('flow')[0];
        if (!flow) {
...
if (document.all && !window.opera) {
    window._$ = function (el) {
        if (typeof el == "string") {
            return window._$(document.getElementById(el));
        }
        else {
